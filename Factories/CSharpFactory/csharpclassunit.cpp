...
CSharpClassUnit::CSharpClassUnit(std::string name)
{
    this->name = name;
...
void CSharpClassUnit::addSubUnit(const std::shared_ptr<Unit> &subUnit, Flags accessType)
{
    std::string key;
    switch (accessType) {
    case PUBLIC:
...
std::vector<std::string> CSharpClassUnit::Compile(unsigned int level)
{
