#include "csharptest.h"


const std::vector<std::string> CSharpTest::output_true_1 ={
                    "class A",
                    "{",
                    "    private int a()",
                    "    {",
...
std::pair<bool, std::vector<std::string>> CSharpTest::runTestCase1()
{
    auto logger = new VectorWriter();
    auto factory = CodeFactory(CodeFactory::c_sharp, logger);
...
void CSharpTest::RunTests(Writer* output_logger)
{
